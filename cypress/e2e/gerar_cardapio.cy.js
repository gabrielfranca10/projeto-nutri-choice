describe("Gerar Cardápio Personalizado", () => {
    ...
    it("Cenário Favorável: Cardápio Gerado com sucesso", () => {
        cy.get('[href="/cardapio/"]').click();
        cy.get('.absolute').click();

    });

    it("Cenário Desfavorável: Cardápio não abre por não responder questionário antes", () => {
        cy.visit('/');
        cy.get('.cadastro').click();
        cy.get('#username').type('Falha Questionario');
        cy.get('#email').type('falha@teste.com');
        cy.get('#senha').type('12345678');
        cy.get('#confirmar_senha').type('12345678');
        cy.get('#data_nascimento').type('2000-01-01');
        cy.get('#genero').type('Masculino');
        cy.get('.bg-lime-500').click();
